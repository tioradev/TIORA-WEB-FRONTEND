...
const PaymentBilling: React.FC = () => {
  ...
  const handlePayPendingCharges = () => {
    const defaultCard = paymentMethods.find(card => card.isDefault && card.isActive);
    if (!defaultCard) {
      alert('Please set a default payment card first.');
      return;
    }

    if (window.confirm(`Pay Rs. ${totalPendingAmount.toFixed(2)} using ${defaultCard.cardType} ****${defaultCard.cardNumber}?`)) {
      // Update pending charges to paid
      setAppointmentCharges(appointmentCharges.map(charge =>
        charge.status === 'pending'
          ? {
              ...
              paidAt: new Date(),
              paymentMethod: `${defaultCard.cardType.charAt(0).toUpperCase() + defaultCard.cardType.slice(1)} ${defaultCard.cardNumber}`
            }
          : charge
      ));
        ...
        <div className="flex space-x-3">
          {pendingCharges.length > 0 && (
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white hover:from-amber-600 hover:to-amber-700 rounded-lg transition-all duration-200"
            >
